...
\usepackage[
    colorlinks=true,
    pdfencoding=auto,
    pdfauthor={Daniel Lemus, Oliver Lee},
    pdftitle={Advanced Dyanmics (wb2630-T1) Q1 2014 Homework Solution No. 8},
    pdftex
]{hyperref}
...
\newpart{5}
\textit{Assume the cart now moves backwards in such a way that
$s_X=-v_0 t$ and $\theta=\pi/8\sin(2\pi f t)$, with the constant parameters
$v_0=\unit[20]{m/s}$ and $f=\unit[2]{Hz}$. The initial $Y$-position $s_Y(t_0)$
of the center of mass $S$ at $t_0=0$ is 0. Find the initial velocity
$\dot{s}_Y(t_0)$ of the cart at $t_0=0$ such that the constraint equation (as
derived in the lecture) is fulfilled. You should do this by replacing the code
in the definition of initial conditions in {\tt simulate\_cart.m}.}
\\

Given the analytic form for the expressions of $s_X(t)$ and $\theta(t)$, we can
take the time derivatives to find expression for $\dot{s}_X(t)$ and $\omega(t)$
which are:
\begin{equation*}
    \dot{s}_X(t) = -v_0 \qquad \omega(t) = \frac{\pi}{8} 2\pi f \cos(2\pi ft)
\end{equation*}
These equations give the following initial conditions:
\begin{align*}
    s_X(t_0) &= 0 \\
    \theta(t_0) &= 0 \\
    \dot{s}_X(t_0) &= -v0 \\
    \omega(t_0) &= \frac{\pi^2 f}{4} = \frac{\pi^2}{2}
\end{align*}
Taking $s_Y(t_0) = 0.2$, the initial condition of $\dot{s}_Y(t_0)$ can be found
using the constraint equation:
\begin{equation*}
    \phi = \dot{s}_X \sin(\theta) - \dot{s}_Y \cos(\theta) + \frac{1}{2}l\omega
        = 0
\end{equation*}
As this constraint is always satisfied, substituting values at $t = t_0$ gives:
\begin{equation*}
    \dot{s}_Y(t_0) = \frac{l\pi^2f}{8} = \frac{\pi^2}{2}
\end{equation*}

The modifications to {\tt simulate\_cart.m} are presented in
\autoref{lst:simcart}.
\\

\newpart{15}
\textit{Write a new function {\tt inversedynamics\_cart.m} that is called by
{\tt simulate\_cart.m} and use it to calculate the applied forces $F_x$ and
$F_y$ in the body-fixed frame that are needed to generate the above movement,
for $N=500$ samples of time (with a sampling rate of $Ts=\unit[0.01]{s}$). What
are the final values for $F_x$ and $F_y$ (so at $t_e=(N-1)\cdot Ts$)? }
\\

In order to determine the inverse dynamics of the cart, the equations of motion
must be re-arranged so that the forces $F_x$ and $F_y$ are on the left hand
side. Starting with the cart equations of motion:
\begin{equation*}
    \begin{pmatrix}
        m & 0 & \frac{-2 I_s}{l}\sin(\theta) \\
        0 & m & \frac{2 I_s}{l}\cos(\theta) \\
        \sin(\theta) & -\cos(\theta) & \frac{l}{2} \\
    \end{pmatrix}
    \begin{pmatrix}
        \ddot{s}_X \\ \ddot{s}_Y \\ \dot{\omega}
    \end{pmatrix}
    =
    \begin{pmatrix}
        F_x \cos(\theta) - 2 F_y \sin(\theta) \\
        F_x \sin(\theta) + 2 F_y \cos(\theta) \\
        -\dot{s}_X \omega \cos(\theta) - \dot{s}_Y \omega \sin(\theta)
    \end{pmatrix}
\end{equation*}
We can ignore the equation of motion for $\dot{\omega}$ and separate the $F_x$
and $F_y$ terms from the forcing vector:
\begin{align*}
    \begin{pmatrix}
        m & 0 & \frac{-2 I_s}{l}\sin(\theta) \\
        0 & m & \frac{2 I_s}{l}\cos(\theta) \\
    ...
    \end{pmatrix}
\end{align*}
As the force coefficient matrix is clearly full rank, we can left multiply by
its inverse to obtain an expression for $F_x$ and $F_y$:
\begin{equation*}
    \begin{pmatrix}
        Fx \\ Fy
    \end{pmatrix}
    ...
    \end{pmatrix}
\end{equation*}
A Matlab implementation is provided in \autoref{lst:invdcart}. See
\autoref{lst:simcart} to see how it is called in {\tt simulate\_cart.m}. \\
The final values of $F_x$ and $F_y$ are:
\begin{equation*}
    \begin{pmatrix} F_x(t_e) \\ F_y(t_e) \end{pmatrix} =
        \begin{pmatrix} 90.0863 \\ -882.048 \end{pmatrix} \unit{N}
\end{equation*}

\begin{lstlisting}[caption={inversedynamics\_cart.m}, label={lst:invdcart}]
...
\newpart{15}
\textit{Add the calculated forces as a matrix with $N$ rows and 2 columns
    (containing the components $F_x$ and $F_y$) as another parameter in the
    Matlab structure {\tt par}.\\\\ Then, with the same initial conditions (also
including the velocity component $\dot{s}_Y(t_0)$ you found in the first part
of this problem), apply these forces again in the forward simulation (by
reading out the matrix entries from {\tt par} in the equations of motion),
using the Modified Euler Method for the $N$ samples of time. Finally, calculate
the absolute error between the final center of mass $X$-component
$\hat{s}_{X}(t_e)$ of the cart, as calculated in the forward simulation, and
the true value $s_X(t_e)=-v_0 t_e$, with $t_e=(N-1)\cdot Ts$.}
\\

Refer to \autoref{lst:simcart} and \autoref{lst:carteom} to see how the force
components are added to {\tt par} and applied in the forward simulation. The
absolute error of $s_X(t_e)$ is found to be:
\begin{equation*}
    |\text{error}| = |s_{X,sim}(t_e) - s_{X,model}(t_e)| =
        |(\unit[-100.631]{m}) - (\unit[-99.8]{m})| = \unit[0.83095]{m}
\end{equation*}

\newpart{15}
\textit{Calculate the constraint force acting on the rear axle of the cart for
    the $N$ samples of your forward simulation. What is the maximum absolute
    value of this constraint force?\\\\
...
Including the constraint equation and adding a lagrange multiplier results in
the following equations of motions:
\begin{equation*}
    \begin{pmatrix}
        \ddot{s}_X \\ \ddot{s}_Y \\ \dot{\omega} \\ \lambda
    \end{pmatrix}
    =
    \frac{2}{4I_s + l^2m} \omega
    \begin{pmatrix}
        -I_s(\sin(2\omega)\dot{s}_X - \cos(2\omega)\dot{s}_Y + \dot{s}_Y) \\
        I_s(\cos(2\omega)\dot{s}_X + \sin(2\omega)\dot{s}_Y + \dot{s}_X) \\
        -lm(\cos(\omega)\dot{s}_X + \sin(\omega)\dot{s}_Y) \\
        -2I_sm(\cos(\omega)\dot{s}_X + \sin(\omega)\dot{s}_Y) \\
    \end{pmatrix}
\end{equation*}

With a closed form solution of $\lambda$, we can calculate the constraint
forces in the $x$ and $y$ directions:
\begin{equation*}
    F_x = \lambda \frac{\partial{\phi}}{\partial{\dot{s}_X}} \qquad
    F_y = \lambda \frac{\partial{\phi}}{\partial{\dot{y}_Y}}
\end{equation*}
\begin{equation*}
    F_x = \lambda \sin(\theta) \qquad F_y = -\lambda \cos(\theta)
\end{equation*}
The constraint forces are then calculated in \autoref{lst:simcart}. \\

Assuming half the mass is distributed above the rear axle, we find the static
friction force to be:
\begin{equation*}
    F_{fr} = \mu \frac{1}{2}mg = (0.7)\frac{1}{2}(20)(9.81) = \unit[68.67]{N}
\end{equation*}
Plotting the calculated constraint force $F_C = \sqrt{F_x^2 + F_y^2}$ with the
static friction force $F_{fr}$ in \autoref{fig:axleforce}, we see that no slip
of the rear wheels is not a good assumption.

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{figures/axle_force.eps}
    \caption{Total Constraint Force on Cart Axle}
    \label{fig:axleforce}
\end{figure}
...
!\colorbox{light-red}{x0 = y0(end,1:3);}!
!\colorbox{light-red}{x\_sim = y(end, 1);}!
!\colorbox{light-red}{x\_mdl = -v0*(N-1)*dt;}!
!\colorbox{light-red}{disp(sprintf('error in sX: \%g - \%g = \%g', x\_sim, x\_mdl, x\_sim - x\_mdl))}!
...
% lambda = -4*Is*m*(sin(q3)*q2' + cos(q3)*q1')*q3'/(4*Is + l**2*m)

!\colorbox{light-red}{dy = zeros(size(y));}!
!\colorbox{light-red}{for i = 1:length(y) - 1}!
!\colorbox{light-red}{    dy(i, :) = cart\_equationsofmotion(t(i), y(i, :), par);}!
!\colorbox{light-red}{end}!

!\colorbox{light-red}{theta\_t = y(:, 3);}!
!\colorbox{light-red}{ds\_X\_t = dy(:, 1);}!
!\colorbox{light-red}{ds\_Y\_t = dy(:, 2);}!
!\colorbox{light-red}{omega\_t = dy(:, 3);}!
!\colorbox{light-red}{lambda\_t = -4*par.Is*par.m*omega\_t/(4*par.Is + l\^2*par.m) .*...}!
!\colorbox{light-red}{    (sin(theta\_t).*ds\_Y\_t + cos(theta\_t).*ds\_X\_t);}!

!\colorbox{light-red}{F\_x = lambda\_t .* sin(theta\_t);}!
!\colorbox{light-red}{F\_y = -lambda\_t .* cos(theta\_t);}!

!\colorbox{light-red}{F\_C = sqrt(F\_x.\^2 + F\_y.\^2);}!
!\colorbox{light-red}{F\_fr = 0.7*par.m/2*9.81;}!

!\colorbox{light-red}{disp(sprintf('static friction force = mu*m/2*g = \%g', F\_fr));}!
!\colorbox{light-red}{xlabel('time [s]')}!
!\colorbox{light-red}{ylabel('force [N]')}!
!\colorbox{light-red}{legend({'F\_C', 'F\_{fr}'})}!
...
\begin{lstlisting}[
    caption={cart\_equationsofmotion.m},
    label={lst:carteom},
    escapechar=!
]
...
!\colorbox{light-red}{dt = 0.01;}!
!\colorbox{light-red}{n = round(t/dt) + 1;}!
!\colorbox{light-red}{Fx = par.F(n, 1);}!
!\colorbox{light-red}{Fy = par.F(n, 2);}!
